...
static __inline__ guint32 _fdb_new_pld(struct fdb* db, const gchar* path, const void* data, const guint16 dlen)
{
  ...
  {
    e_set(E_FATAL|FDB_FULL,"pld file is full");
    longjmp(db->errjmp,1);
  }
...
static __inline__ struct file_idx* _idx_from_id(struct fdb* db, guint32 id)
{
  ...
  if (id > db->lastid)
  {
    e_set(E_FATAL,"invlaid idx id (%d)", id);
    longjmp(db->errjmp,1);
  }
  ...
  if (sum != 0xff)
  {
    e_set(E_FATAL|FDB_CORRUPT,"invlaid checksum of idx entry no.: %d", id);
    longjmp(db->errjmp,1);
  }
#endif
  return idx;
...
static __inline__ struct file_pld* _pld_from_idx(struct fdb* db, struct file_idx* idx)
{
  ...
  {
    e_set(E_FATAL|FDB_CORRUPT,"invlaid pld offset (0x%08X)", idx->off);
    longjmp(db->errjmp,1);
  }
  ...
  if (sum != 0xff)
  {
    e_set(E_FATAL|FDB_CORRUPT,"invlaid checksum of pld entry at offset: 0x%08X", idx->off);
    longjmp(db->errjmp,1);
  }
#endif
  /* checksum pld */
...
static guint32 _fdb_insert_node(struct fdb* db, const struct fdb_file* file)
{
  ...
  if (y->bal == -2)
  {
    ...
    else
    {
      ...
      {
        e_set(E_FATAL|FDB_CORRUPT,"corrupted idx file (broken AVL tree)");
        longjmp(db->errjmp,1);
      }
...
struct fdb* fdb_open(const gchar* path, struct error* e)
{
  ...
  {
    e_set(E_FATAL, "can't create file database directory: %s", path);
    goto err_0;
  }
  ...
  {
    e_set(E_FATAL, "inaccessible file database directory: %s", strerror(errno));
    goto err_0;
  }
...
guint32 fdb_add_file(struct fdb* db, const struct fdb_file* file)
{
  ...
  if (file->path == 0)
  {
    e_set(E_BADARG, "file path must be set at least");
    return 0;
  }
...
guint32 fdb_get_file_id(struct fdb* db, const gchar* path)
{
  ...
  stop_timer(3);
 not_fnd:
  e_set(E_ERROR|FDB_NOTEX, "file not found (%s)", path);
  return 0;
}
...
gint fdb_rem_file(struct fdb* db, const guint32 id)
{
  ...
  if (i->refs == 0)
  {
    e_set(E_ERROR|FDB_NOTEX, "file already has zero refs");
    stop_timer(5);
    return 1;
  }
  i->refs--;
